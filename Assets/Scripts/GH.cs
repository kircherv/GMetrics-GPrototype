...
public class GH : MonoBehaviour
{
       ...
       //    leftClick = false;
        ...
        if (grappleDeployed)
        {
            ...
            else if (Input.GetButton("Down") && !Input.GetButton("Up"))
            {
                if (transform.position.y >= currentGrapplingHook.transform.position.y)
                {
                    AddTension(airTension);
                    GetComponent<Rigidbody>().AddForce(0,accelerationY,0);
                    grapplePayoutSpeed = 0;
                }
                else
                {
                    retracting = -1;
                    grapplePayoutSpeed = 5;
                }
            }
            else
            {
    ...
    public void AddTension(float tension)
    {
        ...
        ropeTension = new Vector3(tension*1.5f,tension,0) + directionToGrapple * Vector3.Dot(customGravity, directionToGrapple) * -1;
